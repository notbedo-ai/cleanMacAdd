import { combine, rowsToTSV } from '../src/parsers';
...
const expectations: Array<[boolean, string]> = [
  ...
  [result.rows.filter((r) => r.port === '').length === 3, '3 continuation rows'],
  ...
  [result.rows[2].port === 'Gi1/0/3' && result.rows[3].port === '', 'Gi1/0/3 continuation row has empty port'],
];
